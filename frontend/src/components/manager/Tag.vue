...
const del = (id) => {
    ElMessageBox.confirm("您确定删除吗？", "确认删除", {
        ...
        request.delete("/admin/tag/delete", { data: { id } })
            .then((res) => {
                if (res.code === 1) {
...
const delBatch = () => {
    ...
    ElMessageBox.confirm(`您确定批量删除这 ${ids.value.length} 条数据吗？`, "确认删除",
        { type: "warning", confirmButtonText: "确认", cancelButtonText: "取消" }).then(() => {
            request.delete("/admin/tag/delete/batch", { data: { ids: ids.value } })
                .then((res) => {
                    if (res.code === 1) {
...
const querySearch = (queryString, cb) => {
    let results = [];
    request.get("/admin/tag/querySearch", {
