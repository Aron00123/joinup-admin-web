    ...
    <div>
      ...
      <div class="table">
        <el-table :data="tableData" stripe @selection-change="handleSelectionChange">
          ...
          <el-table-column prop="name" label="标签名" show-overflow-tooltip />
          <el-table-column prop="author" label="标签描述" show-overflow-tooltip />
      ...
      >
        <el-form :model="form" label-width="150px" style="padding-right: 50px" :rules="rules" ref="formRef">
          ...
          <el-form-item label="标签描述" prop="description">
            <el-input type="textarea" rows="3" v-model="form.description" placeholder="标签描述"></el-input>
          </el-form-item>
  ...
  request
      .post("/tag/listAllTag", {
        headers: {
                ...
                "Authorization": localStorage.getItem("xm-user").token,
            },
      })
      .then((res) => {
...
// 自动补全查询
const querySearchTag = (queryString, cb) => {
  let results = [];
  request
      .post("/tag/querySearch", { queryString })
      .then((res) => {
        if (res.code === "200") {
          results = res.data;
...
const save = () => {
  request
      .post(isHandleAdd.value ? "/admin/tag/add" : "/tag/update", form)
      .then((res) => {
        if (res.code === "200") {
...
const delBatch = () => {
  ...
  ElMessageBox.confirm("您确定批量删除这些数据吗？", "确认删除", {
    ...
    request
        .post("/tag/delete/batch", { ids: ids.value })
        .then((res) => {
          if (res.code === "200") {
            ElMessage.success("操作成功");
            load(1);
